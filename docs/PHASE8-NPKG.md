...
The spec's "npkg install time for a package with 10 dependencies"
benchmark ships with the fixture set: `tests.bench-root` depends on ten
generated leaf packages (`tests.bench-dep-1..10`, generated by
`build/p8-npkg-tests-build.sh`). Run `bash build/p8-t6-bench.sh` after a
deploy; measured on QEMU 8.2 (2 GB / 1 vCPU): **38.2 s for all eleven
packages** - each goes through fetch from the local FAT repository,
Ed25519 signature verification, SHA-256 checksum verification,
