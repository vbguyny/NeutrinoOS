...
#   legs: npkg, drivers, arm64, sdk, repo, samples, archive, ecosystem
param([string[]]$Only = @())
...
if ((Want "drivers") -or (Want "npkg")) {
    Write-Host "`n--- 0/8 npkg test image (shared prereq for driver + npkg legs) ---" -ForegroundColor Yellow
...
if (Want "arm64") {
    Write-Host "`n--- 3/8 ARM64 boot + interrupts + shell input ---" -ForegroundColor Yellow
...
if (Want "drivers") {
    Write-Host "`n--- 2/8 driver framework (device tree binding) ---" -ForegroundColor Yellow
...
if (Want "npkg") {
    Write-Host "`n--- 1/8 npkg package manager ---" -ForegroundColor Yellow
...
if (Want "sdk") {
    Write-Host "`n--- 4/8 SDK: templates + auto-packaging ---" -ForegroundColor Yellow
...
if (Want "repo") {
    Write-Host "`n--- 5/8 repository server (HTTP + signature parity) ---" -ForegroundColor Yellow
...
if (Want "samples") {
    Write-Host "`n--- 6/8 SDK samples (five projects) ---" -ForegroundColor Yellow
...
if (Want "archive") {
    Write-Host "`n--- 7/8 SDK archive ---" -ForegroundColor Yellow
...
if (Want "ecosystem") {
    Write-Host "`n--- 8/8 ecosystem (keys, package templates, docs, site) ---" -ForegroundColor Yellow
